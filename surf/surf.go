...
func (s *SuRF) MarshalSize() int64 {
	return s.ld.MarshalSize() + s.ls.MarshalSize() + s.ld.values.MarshalSize() + s.ls.values.MarshalSize() + s.prefixVec.MarshalSize()
}
...
func (s *SuRF) Unmarshal(b []byte) {
	...
	b = s.ls.values.Unmarshal(b)
	s.prefixVec.Unmarshal(b)
	s.ld.prefixVec = &s.prefixVec
	s.ls.prefixVec = &s.prefixVec
}
